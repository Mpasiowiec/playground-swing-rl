...
swinger_strategy = args.strategy
swings = args.swings_num
...
if swinger_strategy == 'ffm':
      ...
      alpha = np.radians(65)
elif swinger_strategy == 'motionless':
      alpha = np.radians(0)
else:
      print('Unknown strategy of swinging. Possible option: ffm, square-wave, papers-model, motionless') 
      sys.exit()
...
if args.save_data: df.to_csv(os.path.join(args.data_dir, f'sim_data_{swinger_strategy}_n{swings}_t{t_step:.1g}.csv'))
...
if args.off_vis:
      ...
      ani.save(os.path.join(args.vis_dir,f'{swinger_strategy}_n{swings}_t{t_step:.1g}.gif'), writer=PillowWriter(fps=1/t_step))
